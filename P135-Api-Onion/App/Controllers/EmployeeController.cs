...
namespace App.Controllers
{
    ...
    public class EmployeeController : ControllerBase
    {
        ...
        [ProducesResponseType(statusCode: StatusCodes.Status404NotFound)]
        [ProducesResponseType(statusCode: StatusCodes.Status400BadRequest)]
        public async Task<IActionResult> Update([FromRoute][Required] int id,[FromBody] EmployeUpdateDto employe)
        {
            try
            {
                await _service.UpdateAsync(id, employe);
                return Ok();
            }
            catch (ArgumentNullException ex)
            {
        ...
        [ProducesResponseType(statusCode: StatusCodes.Status404NotFound)]
        public async Task<IActionResult> Serach( string serachText)
        {
            try
            {
                
                return Ok(await _service.SerachAsync(serachText));
            }
            catch (ArgumentNullException ex)
            {
                return BadRequest(ex.Message);
